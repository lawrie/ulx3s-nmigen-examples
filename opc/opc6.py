    ...
    def elaborate(self, platform):
        ...
        with m.If(self.clken):
            ...
            with m.Else():
                ...
                m.d.sync += OR_q.eq(Mux(((FSM_q == FET0) | (FSM_q == EXEC)), Repl((op_d == PUSH), 16) ^ Cat([Mux(((op_d == DEC) | (op_d == INC)), self.din[4:8], Cat([op_d == POP, Const(0,3)])), Const(0,12)]), Mux(FSM_q == EAD, RF_w_p2 + OR_q, self.din)))
                with m.If(FSM_q == INT):
                    m.d.sync += [
                ...
                with m.If(((FSM_q == EXEC) & (op != CMP) & (op != CMPC)) | (((FSM_q == WRM) | (FSM_q == RDM)) & IR_q[IRWBK])):
                    m.d.sync += RF_q[IR_q[:4]].eq(Mux((op == JSR), PC_q, result)) 
                with m.If((FSM_q == FET0) | (FSM_q == EXEC)):
